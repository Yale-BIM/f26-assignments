...
Shutter in Part III of this assignment. Then, your goal is to compute the camera's intrinsic parameters ![equation](https://latex.codecogs.com/png.latex?K)<!--$`K`$--> by minimizing:
    ...
    how you implemented your solution in the `calibrate_K.py` script. The online report  allows you to upload an image
...
gather data from a [RealSense camera](https://www.realsenseai.com/products/depth-camera-d435i/), which some versions of Shutter also have on the robot's head.
...
2. Second, the driver for the RealSense D435 camera was started to stream images through the ROS 2 network:
...
4. Fourth, the [depth/save_images.py](depth/save_images.py) script was run to save to disk what the camera was currently observing. The resulting depth (left) and gray (right) images are shown below:
...
To load the data from the image.npz file, you can use the sample script [depth/process_images.py](depth/process_images.py). This script shows you how extract the different components of the `image.npz` file in Python, visualize the images, and select a region of interest in the grayscale image.
